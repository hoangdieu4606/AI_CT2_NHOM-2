...
class App(tk.Tk):
    ...
    def _draw_convergence(self, result: OptimizationResult):
        ...
        xs   = list(range(1, len(result.history_best) + 1))
        ys_b = result.history_best
        ys_a = result.history_avg if result.history_avg else []

        ax.plot(xs, ys_b, color=color, lw=1.8, label="Best")
        if ys_a:
            ax.plot(xs, ys_a, color=DIM, lw=1,
                    ls="--", alpha=0.65, label="Average")

        # ── Trục Y: đảm bảo luôn có khoảng hiển thị hợp lý ──────────────────
        all_vals = list(ys_b) + list(ys_a)
        v_min = min(all_vals) if all_vals else 0
        v_max = max(all_vals) if all_vals else 1

        if v_max - v_min < 1e-6:
            # Tất cả giá trị bằng nhau → mở rộng trục Y ra 10% để thấy đường
            margin = max(abs(v_max) * 0.1, 1.0)
            ax.set_ylim(v_min - margin, v_max + margin)
        else:
            # Bình thường: thêm 5% padding trên dưới
            span   = v_max - v_min
            ax.set_ylim(max(0, v_min - span * 0.05),
                        v_max + span * 0.08)

        # ── Chú thích thêm nếu best = 0 (không có nghiệm hợp lệ) ─────────────
        if v_max == 0:
            ax.text(0.5, 0.5,
                    "Chưa tìm được nghiệm hợp lệ\n(tất cả vượt tải trọng)",
                    transform=ax.transAxes,
                    ha="center", va="center",
                    fontsize=9, color=ACCENT2,
                    bbox=dict(boxstyle="round,pad=0.4",
                              facecolor=SURF2, edgecolor=ACCENT2, alpha=0.8))

        ax.set_title(f"{result.algorithm} — Hội tụ  "
                     f"(best={result.best_value:.1f})",
