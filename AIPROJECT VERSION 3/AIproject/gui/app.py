...
# LƯU Ý: Đảm bảo bạn có sẵn các module này trong project
...
from gui.widgets.animation_canvas import AnimationCanvas
...
class ToggleSwitch(tk.Canvas):
    ...
    def __init__(self, parent, variable: tk.BooleanVar,
                 on_color="#34c759", off_color="#d1d1d6",
                 width=44, height=24, command=None, **kw):
        ...
        # Đã đổi tên để tránh xung đột với self._w của lõi Tkinter
        self._width    = width  
        self._height   = height 
        self._cmd      = command
        self._anim_id  = None
        self._thumb_x  = None   # position of thumb center
    ...
    def _draw(self):
        ...
        w = self._width; h = self._height; r = h // 2
        on = self._var.get()
        bg = self._on_c if on else self._off_c
        # Track
        self._rrect(0, 0, w, h, r, fill=bg, outline="")
        # Thumb
        tx = w - r - 2 if on else r + 2
        self.create_oval(tx - r + 3, 2, tx + r - 3, h - 2,
                         fill="white", outline="")
...
class App(tk.Tk):
    def __init__(self):
        ...
        self._anim_on = tk.BooleanVar(value=False)
    ...
    def _build_truck_area(self):
        ...
        # Nút toggle Animation ON/OFF
        anim_row = _fr(hdr); anim_row.pack(side="right")
        _lbl(anim_row, "Animation", font=FNS, fg=DIM).pack(side="left", padx=(0,4))
        ToggleSwitch(anim_row, variable=self._anim_on,
                     on_color=ACCENT, off_color=BORDER,
                     bg=SURFACE, command=self._on_anim_toggle
                     ).pack(side="left")

        self._truck_algo_lbl = _lbl(hdr, "", font=FNS, fg=DIM)
        self._truck_algo_lbl.pack(side="right", padx=(0,10))

        _sep(p).pack(fill="x", padx=10)

        # Container chứa truck hoặc animation (dùng place để swap)
        self._viz_container = _fr(p, bg=SURFACE)
        self._viz_container.pack(fill="both", expand=True, padx=8, pady=8)

        self._truck = TruckCanvas(self._viz_container)
        self._truck.place(relx=0, rely=0, relwidth=1, relheight=1)
        self._truck.set_solution(self.instance, None)

        self._anim_canvas = AnimationCanvas(self._viz_container, bg=SURF2)
        # ẩn ban đầu
    ...
    def _poll(self):
        try:
            while True:
                ...
                if kind == "ok":
                    # Bọc try-except ở đây để chặn mọi lỗi làm sập giao diện
                    try:
                        cb(payload)
                    except Exception as e:
                        print(f"LỖI ẨN BỊ CHẶN LẠI: {e}")
                        import traceback
                        traceback.print_exc()
                        self._set_status("Lỗi giao diện (đã tự khôi phục).")
                elif kind == "prog":
                    pct, lbl = payload
    ...
    def _on_single_done(self, result: OptimizationResult):
        sliced_best = list(result.history_best)
        sliced_avg = list(result.history_avg) if result.history_avg else []
        opt_idx = 0
        
        if sliced_best:
            best_val = max(sliced_best)
            opt_idx = sliced_best.index(best_val)
            sliced_best = sliced_best[:opt_idx + 1]
            if sliced_avg:
                sliced_avg = sliced_avg[:opt_idx + 1]
        ...
        try:
            ...
            print("Lỗi update Truck:", e)

        self._update_result_text(result)
        self._set_status(f"{result.algorithm} ✓ (Tối ưu tại Gen {opt_idx})")

        n_frames = len(sliced_best)
        speed = min(500, max(15, int(5000 / max(1, n_frames))))
        self._sync_speed = speed

        # Lấy thông số an toàn
        try: n_items_val = int(self._n_items.get())
        except: n_items_val = len(result.best_solution) if result.best_solution else 30

        try:
            if result.algorithm == "GA":
                pop_size_val = int(self.ga_vars["population_size"].get())
            else:
                pop_size_val = int(self.cso_vars["num_cats"].get())
        except: pop_size_val = 20

        self._draw_convergence_sliced(result.algorithm, result.best_value, sliced_best, sliced_avg)

        # --- LUÔN BUILD DATA ANIMATION (để bật toggle bất kỳ lúc nào đều có sẵn) ---
        if sliced_best:
            try:
                anim_on    = hasattr(self, "_anim_on") and self._anim_on.get()
                canvas_obj = getattr(self, "_anim_canvas",
                                     getattr(self, "anim_canvas", None))
                if canvas_obj:
                    if anim_on:
                        self._truck.place_forget()
                        canvas_obj.place(relx=0, rely=0, relwidth=1, relheight=1)
                    else:
                        canvas_obj.place_forget()

                    canvas_obj.start(
                        history_best=sliced_best,
                        best_solution=result.best_solution,
                        algorithm=result.algorithm,
                        speed_ms=speed,
                        n_items=n_items_val,
                        pop_size=pop_size_val,
                        cso_cat_data=getattr(result, "cat_data", None),
                    )
            except Exception as e:
                print("Lỗi chạy animation:", e)


    def _on_anim_toggle(self):
        """
        Toggle animation ON/OFF.
        - BẬT: chỉ HIỆN lại canvas đã render. KHÔNG gọi start() lại.
          Animation chỉ được khởi động từ _on_single_done (khi nhấn Chạy).
        - TẮT: ẩn canvas, khôi phục xe tải.
        """
        canvas_obj = getattr(self, "_anim_canvas", getattr(self, "anim_canvas", None))

        if hasattr(self, "_anim_on") and self._anim_on.get():
            # ── BẬT ──────────────────────────────────────────────────────────
            self._truck.place_forget()
            if canvas_obj:
                canvas_obj.place(relx=0, rely=0, relwidth=1, relheight=1)
            # Không gọi start() ở đây — animation chỉ chạy khi nhấn "Chạy"
        else:
            # ── TẮT ──────────────────────────────────────────────────────────
            if canvas_obj:
                try:
            ...
            # Khôi phục xe tải
            self._truck.place(relx=0, rely=0, relwidth=1, relheight=1)
            if getattr(self, "last_result", None) and getattr(self, "instance", None):
                try:
                    self._truck.set_solution(self.instance,
                                             self.last_result.best_solution)
                except Exception as e:
                    print("Lỗi khi khôi phục xe tải:", e)

    def _draw_convergence_sliced(self, algorithm, best_value, ys_b, ys_a):
        ax = self._ax_single
        ax.clear(); self._style_ax(ax)
        color  = ACCENT if algorithm == "GA" else ACCENT2
        n      = len(ys_b)

        if not self._anim_on.get():
            if n > 0:
                ax.plot(range(1, n+1), ys_b, color=color, lw=1.8, marker="o", markersize=4, label="Best")
            if ys_a:
                ax.plot(range(1, n+1), ys_a, color=DIM, lw=1, ls="--", marker="o", markersize=3, alpha=0.65, label="Average")
            self._finish_chart_sliced(ax, algorithm, best_value, ys_b, ys_a)
            self._canvas_single.draw_idle()
        else:
            self._chart_anim_algo = algorithm
            self._chart_anim_best_val = best_value
            self._chart_anim_ys_b = ys_b
            self._chart_anim_ys_a = ys_a
            self._chart_anim_step   = 1
            self._chart_anim_total  = n
            self._chart_anim_color  = color
            self._chart_anim_tick()

    # === HÀM 3: _chart_anim_tick ===
    def _chart_anim_tick(self):
        if not hasattr(self, "_chart_anim_ys_b"): return
        step  = self._chart_anim_step
        total = self._chart_anim_total
        if step > total: return

        ax = self._ax_single
        ax.clear(); self._style_ax(ax)
        color = self._chart_anim_color
        ys_b  = self._chart_anim_ys_b[:step]
        ys_a  = self._chart_anim_ys_a[:step]
        xs    = range(1, step + 1)

        if ys_b: ax.plot(xs, ys_b, color=color, lw=1.8, marker="o", markersize=4, label="Best")
        if ys_a: ax.plot(xs, ys_a, color=DIM, lw=1, ls="--", marker="o", markersize=3, alpha=0.65, label="Average")
        
        if step == total:
            self._finish_chart_sliced(ax, self._chart_anim_algo, self._chart_anim_best_val, self._chart_anim_ys_b, self._chart_anim_ys_a)
        else:
            ax.set_title(f"{self._chart_anim_algo} — Hội tụ  ({step}/{total})", fontsize=10, color=TEXT, pad=6)
        
        self._canvas_single.draw_idle()

        self._chart_anim_step += 1 
        if self._chart_anim_step <= total:
            speed = getattr(self, "_sync_speed", 50)
            self.after(speed, self._chart_anim_tick)

    # === HÀM 4: _finish_chart (Đổi thành _finish_chart_sliced) ===
    def _finish_chart_sliced(self, ax, algorithm, best_value, ys_b, ys_a):
        n = len(ys_b)
        all_v = list(ys_b) + list(ys_a)
        v_min = min(all_v) if all_v else 0
        v_max = max(all_v) if all_v else 1
        if v_max - v_min < 1e-6:
            margin = max(abs(v_max) * 0.1, 1.0)
            ax.set_ylim(v_min - margin, v_max + margin)
        else:
            span = v_max - v_min
            ax.set_ylim(max(0, v_min - span*0.05), v_max + span*0.08)
        ax.set_title(f"{algorithm} — Hội tụ  (best={best_value:.1f})",
        ...
        ax.margins(x=0.01)
    def _finish_chart(self, ax, result):
        n = len(result.history_best)
        all_v = list(result.history_best) + list(result.history_avg or [])
        v_min = min(all_v) if all_v else 0
        v_max = max(all_v) if all_v else 1
        if v_max - v_min < 1e-6:
            margin = max(abs(v_max) * 0.1, 1.0)
            ...
            span = v_max - v_min
            ax.set_ylim(max(0, v_min - span*0.05), v_max + span*0.08)
        ax.set_title(f"{result.algorithm} — Hội tụ  (best={result.best_value:.1f})",
                     fontsize=10, color=TEXT, pad=6)
        ax.set_xlabel("Generation / Iteration", fontsize=8, color=DIM, labelpad=4)
        ax.set_ylabel("Fitness Value", fontsize=8, color=DIM, labelpad=4)
        ax.legend(fontsize=8, framealpha=0.9, loc="lower right")
    ...
    def _on_bench_done(self, report):
        """Callback khi benchmark hoàn thành. report là dict hoặc list[dict]."""
        self._prog_var.set(100)
        self._prog_lbl.config(text="Hoàn thành!")
        self._set_status("Benchmark xong ✓")

        # run_benchmark_in_memory có thể trả về list (nhiều scenario)
        # hoặc dict (một scenario). Chuẩn hoá về dict để _draw_benchmark dùng.
        if isinstance(report, list):
            if not report:
                messagebox.showinfo("Benchmark", "Không có kết quả.")
                return
            # Nếu trả về list các dict kết quả từng run, tổng hợp lại
            if isinstance(report[0], dict) and "cases" in report[0]:
                # list of report dicts – hiển thị cái đầu tiên
                report = report[0]
            else:
                # list of OptimizationResult – tổng hợp thành report dict
                import numpy as np
                cases = []
                algo_name = "?"
                for r in report:
                    algo_name = getattr(r, "algorithm", algo_name)
                    cases.append({
                        "case_name": getattr(r, "name", f"Run {len(cases)+1}"),
                        "value"    : {
                            "mean": getattr(r, "best_value", 0),
                            "std" : 0.0,
                        },
                        "runtime"  : {
                            "mean": getattr(r, "runtime_sec", 0),
                        },
                    })
                report = {
                    "algorithm"   : algo_name,
                    "scenario_key": self._bench_scenario.get(),
                    "cases"       : cases,
                }

        # Nếu report không có key "cases" → cố gắng xây từ cấu trúc khác
        if not isinstance(report, dict):
            messagebox.showerror("Benchmark", f"Kết quả không hợp lệ: {type(report)}")
            return

        if "cases" not in report:
            # Có thể run_benchmark_in_memory trả về cấu trúc khác
            # Thử lấy từ key phổ biến
            possible_keys = [k for k in report if isinstance(report[k], list)]
            if possible_keys:
                cases_raw = report[possible_keys[0]]
                cases = []
                for item in cases_raw:
                    if isinstance(item, dict):
                        cases.append({
                            "case_name": item.get("name", item.get("case_name", "?")),
                            "value"    : item.get("value", {"mean": item.get("best_value", 0), "std": 0}),
                            "runtime"  : item.get("runtime", {"mean": item.get("runtime_sec", 0)}),
                        })
                report["cases"] = cases
                report.setdefault("algorithm",    self._algo.get())
                report.setdefault("scenario_key", self._bench_scenario.get())
            else:
                messagebox.showinfo("Benchmark", str(report))
                return

        # Cập nhật text kết quả
        if hasattr(self, "_bench_result_text") and self._bench_result_text:
            lines = [f"Scenario : {report.get('scenario_key','?')}\n",
            ...
            for c in report.get("cases", []):
                ...
                lines.append(
                    f"▸ {c.get('case_name','?')}\n"
                    f"   mean={v.get('mean',0):.2f}  std={v.get('std',0):.2f}"
                    f"  time={rt.get('mean',0):.3f}s\n"
                )
...
# ===== THÊM LỆNH NÀY ĐỂ KÍCH HOẠT CHẠY APP =====
